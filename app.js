...
function normalizeFirebaseConfig(rawConfig) {
  ...
  return {
    apiKey: source.apiKey || source.apiKeyapiKey || source.api_key || 'REPLACE_ME',
    authDomain: source.authDomain || source.auth_domain || 'REPLACE_ME.firebaseapp.com',
    projectId: source.projectId || source.project_id || 'REPLACE_ME',
    storageBucket: source.storageBucket || source.storage_bucket || 'REPLACE_ME.appspot.com',
    messagingSenderId: source.messagingSenderId || source.messaging_sender_id || 'REPLACE_ME',
    appId: source.appId || source.app_id || 'REPLACE_ME',
    measurementId: source.measurementId || source.measurement_id || null
  };
}

const FIREBASE_CONFIG = normalizeFirebaseConfig(window.DOOMSTALK_FIREBASE_CONFIG);

const requiredConfigKeys = ['apiKey', 'authDomain', 'projectId', 'storageBucket', 'messagingSenderId', 'appId'];
const missingConfigKeys = requiredConfigKeys.filter((key) => !FIREBASE_CONFIG[key] || String(FIREBASE_CONFIG[key]).startsWith('REPLACE_ME'));
...
const initialState = { users: [], directChats: [], groups: [], notifications: [], messages: [] };
...
function render(checkListeners = true) {
  ...
  if (!configured) {
    root.innerHTML = `<div class="auth-wrap"><div class="auth card"><h2>Firebase config missing</h2><p class="small">Set values in firebase-config.js</p><p class="small">Missing keys: ${missingConfigKeys.join(', ')}</p></div></div>`;
    return;
  }
...
function bindMain(activeChat, meId) {
  ...
  if (msgInput) {
    msgInput.oninput = null;
    msgInput.onblur = null;
  }
